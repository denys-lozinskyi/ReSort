...
def title_maker(file, path2dest_folder):
    '''функция принимает файл дпк, находит в нем имя и фамилию того, кому выдана справка,
       и возвращает их в виде строки, форматируя имя и фамилию в вид: Михаил Васнецов.
       Если в целевой папке имя уже присутствует, возвращает имя и фамилию в формате:
       Михаил Васнецов(n). Если имя в файле найти не удалось, возвращает default_name(n)
    '''
    ...
    if title in os.listdir(path2dest_folder):
        return if_dublicate_title(title, 0, path2dest_folder)
    else:
        return title

        
def if_dublicate_title(file_name, n, path2dest_folder):
    '''функция вызывается в случае, если файл с таким же именем уже присутствует в папке назначения. Функция возвращает принимаемое имя файла с его расширением в формате ИМЯ(n).расширение,
       где n - нумератор повторений имени файла, целое число, начиная с 1, соответствующее количеству повторений имени файла в папке
    '''
    title_constructor = file_name.split('.') #отделяем имя файла от расширения, попутно образовывая список
    title_constructor.append(n) #добавляем в список нумератор повторений
...
for document in files_to_scan:
    if document[-5:] == ".docx":
        ...
        if is_dpk(file):
            ...
            move(path2source_folder + document, path2dest_folder + title_maker(file, path2dest_folder) #перемещение файла в папку с переименованием
